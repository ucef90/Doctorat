...
class AllenCahnProblem:
    ...
    def forcing(self, points: torch.Tensor) -> torch.Tensor:
        ...
        u_t = -u
        u_xx = -(torch.pi**2) * u
        return u_t - self.diffusion * u_xx - self.reaction * (u - u.pow(3))
    ...
    def residual(self, model, points: torch.Tensor, create_graph: bool = True) -> torch.Tensor:
        ...
        return u_t - self.diffusion * u_xx - self.reaction * (u - u.pow(3)) - self.forcing(tx)
    ...
    def component_residuals(
        ...
        residuals = {
            ...
            "initial": model(sets.initial) - self.exact(sets.initial),
