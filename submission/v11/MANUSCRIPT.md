...
Adaptive collocation changes the training measure observed by global loss
...
forward and inverse differential-equation problems [@raissi2019]. Gradient-flow pathologies
and adaptive balancing were subsequently analyzed by Wang, Teng and Perdikaris
and by several self-adaptive weighting methods [@wang2021]. Wu et al. provided a broad
comparison of nonadaptive and residual-based adaptive collocation [@wu2023]. Nabian,
...
kernel density estimation [@song2025]. Recent work has gone further: joint weighting and
...
potentials and variational objectives [@toscano2026].
...
adaptive training set. ReCoA-PINN (M6) uses the same adaptive-sampling protocol as M5 but computes
...
### Fixed-reference controller and interpretation
...
### Scope of exact proposal correction
...
## V09–V10 extensions and common-loss audit
...
## Architecture, budgets and observation counts
...
: Base architectures and optimization budgets.
...
Sections 4.1–4.6 report historical configurations and references. New V09–V10 evidence follows in Sections 4.7–4.11. Historical noisy-data rankings are recipe comparisons subject to the loss confound described in Section 3.5.

## Fixed-reference decoupling

On long Burgers runs, M5 shows a positive concentration–gradient-gap
correlation in all ten seeds. Nevertheless, M6 does not show a conclusive improvement in
accuracy under the reported paired interval: the paired median difference M6−M5 is \(-0.0291\), with 95% interval
...
## Exact correction and its matched control
...
## Comparison with KDE volume weighting
...
## Variational residual-based attention

The protocol-frozen common-backbone vRBA baseline completes all runs without
...
## Local/global decomposition at the V09 budgets
...
## Sparse and corrupted observations under a common loss
...
## Re-running the historical baselines with a common observation loss
...
The experiments evaluate the original fixed-reference hypothesis and separate three statements that are often conflated. First,
...
8. Local/global and potential ablations, spatial maps and doubled-step sensitivity are available. Common-loss noisy baseline reruns are also available. Equal-time comparisons remain outside the claims of this paper. A separate-host portability protocol is supplied; its execution status is reported in the supplement.
...
behavior depends on the measure used to estimate each loss component. The tested fixed-reference ReCoA-PINN implementation does not establish general stabilization across the evaluated configurations. Exact proposal
...
All journal-specific declarations require author confirmation before submission.
